...
if not Redis("PM_TEXT"):
    UNAPPROVED_MSG = (
        "**Seguridad Privada de {ON}!**\n\n{UND}\n\nTienes {warn}/{twarn} Advertencias!"
    )
else:
    UNAPPROVED_MSG = (
        "**Seguridad Privada de {ON}!**\n\n"
...
async def _(e):
    if not e.is_private:
        return await eor(e, "`Usame en pv.`", time=3)
...
if sett == "True":
        ...
        if not is_approved(user.id) and event.text != UND:
            ...
            try:
                ...
                await asst.edit_message(
                    ...
                    f"Pm de  **{mention}** [`{user.id}`] con **{wrn}/{WARNS}** Advertencias!",
                    buttons=[
                ...
                _not_approved[user.id] = await asst.send_message(
                    int(udB.get("LOG_CHANNEL")),
                    f"Pm de **{mention}** [`{user.id}`] con **1/{WARNS}** Advertencias!",
                    buttons=[
                        Button.inline("aprobar PM", data=f"approve_{user.id}"),
                        Button.inline("Bloquear PM", data=f"block_{user.id}"),
                    ],
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    if "Seguridad Privada" in event.text or "**Seguridad Privada" in event.text:
                        return
    ...
    async def approvepm(apprvpm):
        ...
        if not is_approved(user.id):
            ...
            await eod(
                apprvpm,
                f"[{user.first_name}](tg://user?id={user.id}) `Ha sido aprobado!`",
            )
            try:
                await asst.edit_message(
                    int(udB.get("LOG_CHANNEL")),
                    ...
                    buttons=[
                        Button.inline("Desaprobar PM", data=f"disapprove_{user.id}"),
                        Button.inline("Bloquear", data=f"block_{user.id}"),
                    ],
                )
            except KeyError:
                _not_approved[user.id] = await asst.send_message(
            ...
            await eor(apprvpm, "`Usuario aprobado antes.`", time=5)
    ...
    async def disapprovepm(e):
        ...
        if is_approved(user.id):
            ...
            await eod(
                e, f"[{user.first_name}](tg://user?id={user.id}) `Desaprobado en PM!`"
            )
            try:
                await asst.edit_message(
                    int(udB.get("LOG_CHANNEL")),
                    _not_approved[user.id],
                    f"#Desaprobado\n\n[{user.first_name}](tg://user?id={user.id}) [{user.id}] ` Desaprobado para pm.`",
                    buttons=[
            ...
            await eod(
                e, f"[{user.first_name}](tg://user?id={user.id}) Nunca fue aprobado!"
            )
...
async def blockpm(block):
    ...
    try:
        await asst.edit_message(
            int(udB.get("LOG_CHANNEL")),
            _not_approved[user],
            f"#Bloqueado\n\n[{aname.first_name}](tg://user?id={user}) [`{user}`] Ha sido **Bloqueado**.",
            buttons=[
...
async def unblockpm(unblock):
    ...
    if match == "all":
        ...
        while u_s.users:
            ...
            for user in u_s.users:
                ...
                await unblock.client(UnblockRequest(user.id))
            count += len(u_s.users)
        return await eor(msg, f"__desbloqueados {count} users.__")
    try:
        await unblock.client(UnblockRequest(user))
        aname = await unblock.client.get_entity(user)
        await eor(unblock, f"`{aname.first_name} ha sido desbloqueado!`")
    ...
    try:
        await asst.edit_message(
            int(udB.get("LOG_CHANNEL")),
            _not_approved[user],
            f"#desbloqueado\n\n[{aname.first_name}](tg://user?id={user}) [`{user}`] desbloqueado **desbloqueado**.",
            buttons=[
...
async def apr_in(event):
    ...
    if not is_approved(uid):
        ...
        try:
            ...
            user_name = ""
        await event.edit(
            f"#APPROVED\n\n[{user_name}](tg://user?id={uid}) [`{uid}`] ` Aprobado a pm!`",
            buttons=[
                [
                    Button.inline("Desaprobar PM", data=f"disapprove_{uid}"),
...
async def disapr_in(event):
    ...
    if is_approved(uid):
        ...
        try:
            ...
            user_name = ""
        await event.edit(
            f"#DISAPPROVED\n\n**[{user_name}](tg://user?id={uid})** [`{uid}`] `Desaprobado de PMs!`",
            buttons=[
                [
                    Button.inline("aprobar PM", data=f"approve_{uid}"),
                    Button.inline("Block", data=f"block_{uid}"),
                ],
            ],
        )
        ...
        await event.edit(
            "`Usuario nunca aprobado!`",
            buttons=[
                [
                    Button.inline("Desaprobado PM", data=f"disapprove_{uid}"),
                    Button.inline("Block", data=f"block_{uid}"),
                ],
            ],
        )
...
async def blck_in(event):
    ...
    await event.edit(
        f"#BLOCKED\n\n[{user_name}](tg://user?id={uid}) [`{uid}`] ha sido **Bloqueado!**",
        buttons=[
