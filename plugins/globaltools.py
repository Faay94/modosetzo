...
async def _(e):
    xx = await eor(e, "`Perdonando a esta nena, desbaneo global en camino, bebé es tu única oportunidad ❤️...`")
    ...
    try:
        ...
        pass
    await xx.edit(
        f"`Desbaneo global de` [{name}](tg://user?id={userid}) `en al menos {chats} chats.\nRemovido de tu puta madre, digo, de la lista de gbaneos.`",
    )


@ultroid_cmd(pattern="gban ?(.*)", fullsudo=True)
async def _(e):
    xx = await eor(e, "`Adiós puta vas gban...`")
    reason = ""
    if e.reply_to_msg_id:
        userid = (await e.get_reply_message()).sender_id
        try:
    ...
    try:
        ...
        pass
    gb_msg = f"**#Puta Gbaneada** [{name}](tg://user?id={userid}) `en {chats} chats hija de tu puta madre, valiste verga!`"
    if reason:
        gb_msg += f"\n**Razón** - {reason}"
...
async def gkick(e):
    xx = await eor(e, "`Expulsando al homosexual de todos los grupos...`")
    if e.reply_to_msg_id:
        userid = (await e.get_reply_message()).sender_id
    elif e.pattern_match.group(1):
        userid = await get_user_id(e.pattern_match.group(1))
    ...
    async for gkick in e.client.iter_dialogs():
        if gkick.is_group or gkick.is_channel:
            try:
                ...
                pass
    await xx.edit(f"`He expulsado a` [{name}](tg://user?id={userid}) `de {chats} grupos.`")


@ultroid_cmd(pattern="gmute ?(.*)", fullsudo=True)
async def _(e):
    xx = await eor(e, "`Muteando al mk de los grupos...`")
    if e.reply_to_msg_id:
        userid = (await e.get_reply_message()).sender_id
    elif e.pattern_match.group(1):
        userid = await get_user_id(e.pattern_match.group(1))
    ...
    if is_gmuted(userid):
        return await eor(xx, "`el cv ya está muteado.`", time=4)
    async for onmute in e.client.iter_dialogs():
        if onmute.is_group:
            try:
                ...
                pass
    gmute(userid)
    await xx.edit(f"`muteado` [{name}](tg://user?id={userid}) `de {chats} chats.`")


@ultroid_cmd(pattern="ungmute ?(.*)", fullsudo=True)
async def _(e):
    xx = await eor(e, "`Quitando el muteo global...`")
    if e.reply_to_msg_id:
        userid = (await e.get_reply_message()).sender_id
    elif e.pattern_match.group(1):
        userid = await get_user_id(e.pattern_match.group(1))
    ...
    if not is_gmuted(userid):
        return await eor(xx, "`El cv no está muteado`", time=3)
    async for hurr in e.client.iter_dialogs():
        if hurr.is_group:
            try:
                ...
                pass
    ungmute(userid)
    await xx.edit(f"`Desmuteado` [{name}](tg://user?id={userid}) `en {chats} chats.`")
...
async def list_gengbanned(event):
    ...
    msg = ""
    if not users:
        return await x.edit("`No has baneado a nadie!`")
    for i in users:
        try:
            name = (await ultroid_bot.get_entity(int(i))).first_name
        except BaseException:
            name = i
        msg += f"<strong>Usuario</strong>: <a href=tg://user?id={i}>{name}</a>\n"
        reason = users[i]
        msg += f"<strong>Razón</strong>: {reason}\n\n" if reason is not None else "\n"
    gbanned_users = f"<strong>Lista de baneados por  {OWNER_NAME}</strong>:\n\n{msg}"
    if len(gbanned_users) > 4096:
        ...
        await x.reply(
            file="gbanned.txt",
            message=f"Lista de baneados por [{OWNER_NAME}](tg://user?id={OWNER_ID})",
        )
