...
SUP_BUTTONS = [
    [
        Button.url("• Owner •", url="https://t.me/setzolandv2_owner"),
        Button.url("• Team 50% •", url="t.me/team_50"),
    ],
]
...
async def inline_alive(o):
    ...
    WEB0 = InputWebDocument(
        "https://telegra.ph/file/092497b07b34768d520d4.png", 0, "image/png", []
    )
    RES = [
    ...
    await o.answer(RES, switch_pm="Portal", switch_pm_param="start")
...
async def inline_handler(event):
    ...
    if INLINE_PIC:
        ...
        result = await event.builder.article(
            title="Menu de ayuda", text=text, buttons=_main_help_menu
        )
    ...
    await event.edit(
        ...
        buttons=[
            [
                ...
                Button.inline("•Tiempo en linea•", data="upp"),
            ],
            [
                Button.inline("•Estadisticas•", data="alive"),
                Button.inline("•Actualizar•", data="doupdate"),
            ],
            [Button.inline("« Volver", data="open")],
        ],
    )
    ...
    if len(changelog_str) > 1024:
        ...
        await event.edit(
            ...
            buttons=[
                ...
                [Button.inline("« Volver", data="ownr")],
            ],
        )
...
async def on_plug_in_callback_query_handler(event):
    ...
    try:
        ...
        pass
    if help_string == "":
        reply_pop_up_alert = f"{plugin_name} No tiene ayuda especifica..."
    else:
    ...
    buttons.append(
        [
            Button.inline("« Volver", data="back"),
        ]
    )
    try:
...
async def on_vc_plg_callback_query_handler(event):
    ...
    try:
        ...
        pass
    if help_string == "**Comandos disponibles:**\n\n":
        reply_pop_up_alert = f"{plugin_name} Ayuda no detallada..."
    else:
        reply_pop_up_alert = help_string
    reply_pop_up_alert += ""
    buttons = []
    ...
    buttons.append(
        [
            Button.inline("« Volver", data="vc_helper"),
        ]
    )
    try:
        if str(event.query.user_id) in owner_and_sudos():
...
async def on_plug_in_callback_query_handler(event):
    ...
    buttons.append(
        [
            Button.inline("« Volver", data="buck"),
        ]
    )
    try:
        if str(event.query.user_id) in owner_and_sudos():
